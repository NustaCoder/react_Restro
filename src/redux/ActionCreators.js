...
export const postFeedback = (firstname, lastname, telnum, email, agree, contactType, message) => (dispatch) => {
    ...
    return fetch(baseUrl + 'feedback', {
        ...
        .then(response => response.json())
        .then(response => dispatch(alert(" firsname : " + response.firstname + " lastname :" + response.lastname + " telnum : " + response.telnum + " email : " + response.email + " agree : " + response.agree + "contactType :" + response.contactType + " message : " + response.message + " id : " + response.id8)))
        .catch(error => { console.log('post comments', error.message); alert('Your comment could not be posted\nError: ' + error.message); });
};
